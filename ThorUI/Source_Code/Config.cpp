...
void Config_Array::AddVec2(const Vec2& vec)
{
	json_array_append_number(arr, vec.x);
	json_array_append_number(arr, vec.y);
	size += 2;
}

void Config_Array::AddColor(const Color& color)
{
...
Vec2 Config_Array::GetVec2(uint index, Vec2 default) const
{
	index *= 2;
	if (index + 1 < size)

	{
		return (Vec2(GetNumber(index, default.x), GetNumber(index + 1, default.y)));
	}
	else
	{
		LOG("[Warning] JSON Array: Index out of size");
